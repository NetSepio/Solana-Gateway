...
import (
	...
	"github.com/NetSepio/solana-gateway/api/v1/flowid"
	"github.com/NetSepio/solana-gateway/app/stage/appinit"
	"github.com/NetSepio/solana-gateway/pkg/testingcommon"
...
func getSignature(eula string, flowId string, testWallet testingcommon.TestWallet) string {
	...
	solSign, err := testWallet.PrivKey.Sign([]byte(messageData))
	if err != nil {
		log.Fatal(err)
	}
	return hex.EncodeToString(solSign.ToSlice())
}
